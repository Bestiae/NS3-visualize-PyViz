...
class Visualizer(GObject.GObject):
    ...
    def __init__(self):
        ...
        #for change button to off and on
    ...
    def create_gui(self):
        ...
        screenrecord_button = GObject.new(Gtk.ToggleButton,
                                          label="Screen Record OFF",
                                          relief=Gtk.ReliefStyle.NONE, focus_on_click=True,
                                          visible=True)
        hbox.pack_start(screenrecord_button, False, False, 4)
        ...
        load_button_icon(screenrecord_button, "applets-screenshooter")
        screenrecord_button.connect("clicked", self.do_record)
        ...
        # shortcut button F3 in feature, a group of keyboard accelerators
        accel_group = Gtk.AccelGroup()
        # добавляет группу клавиатурных ускорителей(accelerators) для окна.
        self.window.add_accel_group(accel_group)

        # устанавливает ускоритель(accelerator) для созданой клавиши
        self.play_button.add_accelerator("clicked", accel_group,
                                         Gdk.KEY_F3, 0, Gtk.AccelFlags.VISIBLE)
        # после нажатия клавиши вызывается  self._start_update_timer() в функции _on_play_button_toggled
        # я так понимаю, что эта строка определяет то, что прога не зависает а продолжает работать.
        self.play_button.connect("toggled", self._on_play_button_toggled)
        # (HBox) связано это с тем, что в форме(окне) можно поставить что-то, pack_start отвечает за расположение объекта внктри бокса
        ...
        # добавляет "Advanced" функционал в окно
        vbox.pack_start(self._create_advanced_controls(), False, False, 4)

        # в 2014 году этого не было в программе PyViz
        # это добавлено позже в NS3 core.py {
    ...
    def _screen_record(self, dummy_butto, i):
        """
        эта функция делает скриншот и сохраняет его в нашей созданой tmp директории
        под тем, что это .pdf
        """
        #print "Cheese!"
        # file_name = self._get_export_file_name_record()
        # file_name = '/home/bestiae/ns-3/test_petro/video'
        ...
        # self.i += 1
        new_name = f'{self._svg_dirpath}/file_{a}{extension}'
        # new_name = f'{self._file_name}_{a}{extension}'
            # это сделает мне сразу .png вместо .pdf
            # surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 100, 100)
            # context = cairo.Context(surface)
            # drow something and then
            # surface.write_to_png('filename')
        # surface = cairo.SVGSurface(new_name, dest_width, dest_height)
        # sur = cairo.ImageSurface(cairo.FORMAT_RGB24, dest_width, dest_height)
        surface = cairo.PDFSurface(new_name, dest_width, dest_height)
    ...
    def update_view_timeout(self):
        """
        эта функция вызывается, когда мы запустили нашу визуализацию.
        эта функция вызывается каждый раз?? пока клавиша "Simulate F3" не будет нажата снова.
        то есть она оновляет view_timeout что и запускает вырисовывание симуляции в окне визуализатора.

        я заметил, что когда ты изменяешь таймер, то эта функция также запускается, из чего можно сделать вывод,
        что таймер если чтоит на 1,000 и мы нажимаем "Simulate F3", то пока клавиша "Simulate F3" запущена,
        то таймер оновляется с интервалм 1,000

        я добавил в эту функцию свой иф. Иф работает такм образом, что когда клавиша "Screen record" запущена
        то при вызове этой функции делается скриншот в нашу tmp директорию. А если клавиша не нажата, то просто
        делается основной код функции без запуска _screen_record()
        """
        if self.on_record:
            self._screen_record(None, None)
    ...
    def _start_update_timer(self):
        ...
        # тут начинается скрипт старта симуляции
        # здесь вызвать вункцию скрин рекорд
    ...
    def _on_play_button_toggled(self, button):
        # если мы клавишу симуляции запустили, то делается _start_update_timer() в которой
        # работает и запускается визуализация симуляции
        if button.get_active():
            # start simulation
            self._start_update_timer()
        else:
            # self._take_screenrecord(None, 1)
            if self._update_timeout_id is not None:
    ...
    def _take_screenshot(self, dummy_butto):
        #print "Cheese!"
    ...
    def post_record(self):
        print("processing..............")
        """
        эта функция запускает скрипт, который делает .avi, mp4, .gif в выбраном интервале.
        в этот скрипт мы передаем 2 аргументы это: _svg_dirpath и _file_name
        """
        # we do not use it for our program, it's just test!
        pathlist = Path(self._svg_dirpath).glob('*.pdf')
        # print('this is pathlist: ', pathlist)
        for path in pathlist:
            # because path is object not string
            path_in_str = str(path)
            # print(path_in_str)
        # just test below here
        # получаем корень нашей программы NS3
        root = Path(__file__).parents[3]
        # print("this is root: ", root)
        # print("this is _file_name: ", self._file_name)
        # print("this is _svg_dirpath: ", self._svg_dirpath)

        os.system(f'python3 {root}/make_record.py {self._svg_dirpath} {self._file_name}')


    def do_record(self, screenrecord_button):
        """
        изначально создается в else временная папка, после чего запускается функция _pre_record(),
        потом, после нажатия клавиши повторно клавиша настраивается на False, запускается функция post_record(),
        а после удаляется временная директория.
        """
        if self.on_record:  # switch from ON to OFF
            self.on_record = False
            print("Just wait a while.. ")
            self.post_record()
            # screenrecord_button.set_label("Screen Record")
            dialog = Gtk.MessageDialog(parent=self.canvas.get_toplevel(),
                                       flags=Gtk.DialogFlags.DESTROY_WITH_PARENT,
                                       type=Gtk.MessageType.WARNING,
                                       buttons=Gtk.ButtonsType.OK,
                                       message_format="Your file are saved on directory: %s" % (self._file_name))
            dialog.run()
            dialog.destroy()
            shutil.rmtree(self._svg_dirpath)
            screenrecord_button.set_label("Screen Record OFF")
            # print("second click, this is on_record: ", self.on_record)
        else:  # switch from OFF to ON
            screenrecord_button.set_label("Screen Record ON")
            self.on_record = True
            # self.i = 0
            self._svg_dirpath = tempfile.mkdtemp()
            # print("this is _svg_dirpath: ", self._svg_dirpath)
            # ... do stuff with dirpath
            self._pre_record()
            print("Please press the button Simulate (F3), to start Record the visualization of the simulation.")
            dialog = Gtk.MessageDialog(parent=self.canvas.get_toplevel(),
                                       flags=Gtk.DialogFlags.DESTROY_WITH_PARENT,
                                       type=Gtk.MessageType.WARNING,
                                       buttons=Gtk.ButtonsType.OK,
                                       message_format="Please press the button Simulate (F3), to start Record the visualization of the simulation.")
            dialog.run()
            dialog.destroy()
            # return
            # print("first click, this is on_record: ", self.on_record)

    def _pre_record(self):
        """
        эта функция создает окно, в котором мы выбираем путь и записываем формат файла который хотим сохранить.
        Есть только 3 варианта: .gif, .avi, .mp4
        функция также записывает в переменную _file_name полный путь который мы выбрали в этом окне.
        после чего это окно удаляется.
        """
        sel = Gtk.FileChooserDialog("Save...", self.canvas.get_toplevel(),
                                    Gtk.FileChooserAction.SAVE,
                                    (Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,
        ...
        sel.set_local_only(True)
        sel.set_do_overwrite_confirmation(True)
        sel.set_current_name("Unnamed")

        filter = Gtk.FileFilter()
        filter.set_name("Video (.avi or .mp4)")
        ...
        # print("this is self._file_name: ", self._file_name)
